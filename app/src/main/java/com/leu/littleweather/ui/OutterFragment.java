 ...
 */
public class OutterFragment extends BaseFragment {
     ...
     * @param view
     */
    private void getForecastSetUi(boolean ifRefresh,View view){
        //是否刷新,是的话从网络获取
        if (ifRefresh){
            mForecast=mForecastBiz.getRefreshForecast(mCity,CommonUtil.IsNetAvailable(getActivity()));
        }
        mForecast=mForecastBiz.getCityForecast(mCity, CommonUtil.IsNetAvailable(getActivity()));
        //如果返回的城市天气为null，就什么都不做
        if (mForecast==null){
            return;
        }
        //否则开始设置UI
    ...
    {   //今天的天气碎片
        max=mForecast.getDaily_1_max();
        min=mForecast.getDaily_1_min();
        FragmentManager fm = getFragmentManager();
        FragmentTransaction transaction = fm.beginTransaction();
        mBottomOne = new BottomFragmentOne().newInstanceOne(max,min);
